...
app.post('/submit', async (req, res) => {
  ...
  try {
    ...
    const launchOptions = {
      headless: 'new',
      // 도커 환경변수가 있으면 우선 사용 (Dockerfile에서 설정함)
      executablePath: process.env.PUPPETEER_EXECUTABLE_PATH || undefined,
      args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage']
    };

    // 도커가 아닐 때(로컬 등)를 위한 백업 로직
    if (!launchOptions.executablePath && process.platform === 'linux') {
      launchOptions.executablePath = '/usr/bin/chromium-browser';
    } else if (!launchOptions.executablePath && process.platform === 'win32') {
      // 윈도우 경로 찾기 로직
      const possiblePaths = [
    ...
    res.json({
      ...
      submissionId: `${studentId}_${Date.now()}`,
      pdfBase64: pdfBase64
    });
